...
from bipedal.assets.bipedal import BIPEDAL_CFG, BIPEDAL_CFG_V1  # isort:skip
...
class BipedalSceneCfg(InteractiveSceneCfg):
    ...
    # cartpole
    robot: ArticulationCfg = BIPEDAL_CFG_V1.replace(prim_path="{ENV_REGEX_NS}/robot")
    # sensors
...
class ActionsCfg:
    ...
    thigh_effort = mdp.JointEffortActionCfg(asset_name="robot", joint_names=[".*_hip2thigh"], scale=1.0)
    calf_effort = mdp.JointEffortActionCfg(asset_name="robot", joint_names=[".*_thigh2calf"], scale=1.0)
    wheel_effort = mdp.JointEffortActionCfg(asset_name="robot", joint_names=[".*_calf2wheel"], scale=1.0)
...
@configclass
class BipedalEnvCfgV1(ManagerBasedRLEnvCfg):
